...
pub fn main() {
    ...
    let window = video_subsystem.window("rust nes demo", 256 * 3, 240 * 3)
        .position_centered()
        .build()
        .unwrap();
    ...
    'running: loop {
        ...
        canvas.copy(&texture, None, Some(Rect::new(10, 10, 266, 250))).unwrap();
        canvas.present();

        let elapsed_time = SystemTime::now().duration_since(prev_time).unwrap().as_nanos();
